...
vector<vector<int>> multiply_matrix(vector<vector<int>> matrix_A, vector<vector<int>> matrix_B) {
    ...
    */
    ...
    //Crea el vector resultante y le asignamos el resultado de la multiplicación de matrices
    vector<vector<int>> res(row_1, vector<int>(col_2, 0));  //O(row_1 * row_2)   

    for (int i = 0; i < row_1; ++i) {                       //O(row_1)      
        for (int j = 0; j < col_2; ++j) {                       //O(col_2)
            for (int k = 0; k < col_1; ++k) {                       //O(col_1) = O(row_2)
